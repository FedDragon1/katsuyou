...
const TableLink: FC<TableLinkProps> = ({ href }) => {
    return (
        <Link href={href} className={"group relative left-0 hover:left-1 transition-all"}>
...
const TableRow: FC<TableRowProps> = ({ index, date, duration, name, accuracy, children, heading, disabled }) => {
    const bold = heading ? 'font-semibold' : ''
    const grey = disabled ? 'text-zinc-300' : ''

    return (
        <div className={`w-full flex min-[1700px]:gap-20 sm:gap-10 gap-4 text-lg md:text-xl border-b border-zinc-700 py-10 items-center ${grey}`}>
            ...
            <span className={`basis-0 ${bold} text-right md:text-left md:min-w-[150px] xl:pl-8`}>{children}</span>
        </div>
    )
}

const LearningHistoryTable: FC<TableProps> = async ({ min, max }) => {
    ...
    return (
        <div className={"flex flex-col"}>
            <TableRow index={"#"} date={"Date"} duration={"Duration"} name={"Activity"} accuracy={"Accuracy"}
                      heading={true}>
                Link
            </TableRow>
            ...
            {Array(Math.max(minItems - data.length, 0)).fill(0).map((_, i) => {
                return (
                    <TableRow index={"--"} date={"--"} duration={"--:--"} name={"--"} accuracy={"--%"} key={i} disabled={true}>
